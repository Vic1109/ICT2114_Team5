...
# Backward compatibility class that matches the original interface
class SmartSSHLogReaderLegacy:
    """Legacy interface for backward compatibility"""
    
    def __init__(self):
        # These would be set from config in the main application
        self.host = None
        self.username = None
        self.password = None
        self.port = 22
        self.alerts_path = "/var/ossec/logs/alerts/alerts.json"
        self.archives_base = "/var/ossec/logs/archives"
        
        self._reader = None
    
    def _ensure_reader(self):
        """Ensure the reader is initialized with current config"""
        if self._reader is None and all([self.host, self.username, self.password]):
            self._reader = SmartSSHLogReader(
                self.host, self.username, self.password, self.port,
                self.alerts_path, self.archives_base
            )
    
    def connect(self) -> bool:
        """Establish SSH connection"""
        self._ensure_reader()
        return self._reader.connect() if self._reader else False
    ...
    def read_alerts(self) -> List[Dict]:
        """Read current alerts from alerts.json"""
        self._ensure_reader()
        return self._reader.read_alerts() if self._reader else []
    
    def read_archives_smart(self, past_days: int = 7) -> List[Dict]:
        """Read archive logs with smart date boundary handling"""
        self._ensure_reader()
        return self._reader.read_archives_smart(past_days) if self._reader else []
    
    def get_smart_archive_dates(self, past_days: int) -> List[datetime]:
        """Generate smart date list that handles month/year boundaries"""
        self._ensure_reader()
        return self._reader.archive_reader.get_smart_archive_dates(past_days) if self._reader else []
